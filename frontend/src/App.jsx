...
function App() {
  ...
  const [persistentAlerts, setPersistentAlerts] = useState([]); // New state for persistent alerts
  
  const videoRef = useRef(null);
  const canvasRef = useRef(null);
  const streamRef = useRef(null);
  const alertTimeoutRef = useRef(null); // Ref to manage alert timeout
  ...
  const stopCamera = () => {
    ...
    // Clear persistent alerts when camera stops
    setPersistentAlerts([]);
  };
  ...
  const captureAndDetect = async () => {
    ...
    canvas.toBlob(async (blob) => {
      ...
      try {
        ...
        // Handle alerts with confidence levels
        if (detections.length > 0) {
          // Add new detections to persistent alerts
          ...
          setPersistentAlerts(prevAlerts => {
            // Filter out old alerts (older than 5 seconds)
          ...
          // Set temporary alert message that will clear after a delay
          const alertText = detections.map(d => 
            `${d.class} (${(d.confidence * 100).toFixed(1)}%)`
          ).join(', ');
          setAlertMessage(`${alertText} detected!`);
          
          // Clear the temporary alert after 3 seconds
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      // Handle alerts with confidence levels for file upload
      if (detections.length > 0) {
        const alertText = detections.map(d => 
          `${d.class} (${(d.confidence * 100).toFixed(1)}%)`
        ).join(', ');
  ...
  const switchModel = async (model) => {
    try {
      ...
      await axios.post('/api/models/switch', formData);
      setCurrentModel(model);
      setAlertMessage(`Switched to ${model} detection model`);
      // Clear persistent alerts when model switches
      setPersistentAlerts([]);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white transition-colors duration-200">
      ...
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        {/* Persistent Detection Alerts */}
        {persistentAlerts.length > 0 && (
          <div className="mb-8 p-4 bg-red-100 dark:bg-red-900 border-l-4 border-red-500 text-red-700 dark:text-red-200">
            <p className="font-bold">Recent Detections:</p>
            <ul className="list-disc pl-5 mt-2">
              {persistentAlerts.map((alert, index) => (
                <li key={index}>
                  {alert.class} detected with {Math.round(alert.confidence * 100)}% confidence
                </li>
              ))}
            </ul>
          </div>
